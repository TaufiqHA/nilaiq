...
@section('content')
<!-- Container Utama Catatan Wali Kelas -->
<div class="p-0 sm:p-6 border-0 sm:border border-default border-dashed rounded-none sm:rounded-base bg-transparent sm:bg-white/40 dark:sm:bg-neutral-secondary-medium/20 backdrop-blur-none sm:backdrop-blur-md space-y-4 sm:space-y-6 w-full">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse text-xs">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center space-x-1.5">
                            ...
                            <span class="inline-flex items-center text-xs font-bold text-heading">Catatan Wali Kelas</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <h1 class="text-2xl font-extrabold tracking-tight text-heading">Catatan Wali Kelas</h1>
            <p class="text-xs text-body mt-0.5">Kelola data catatan perkembangan, motivasi, dan rekomendasi wali kelas untuk setiap siswa.</p>
        </div>
    ...
    @php
        ...
        $hasCatatanCount = $students->filter(function($s) {
            return !empty($s->catatanWaliKelas?->catatan);
        })->count();
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div class="p-3.5 bg-white dark:bg-neutral-primary-soft border border-default rounded-base shadow-xs flex items-center gap-3">
            ...
            <div>
                <p class="text-[11px] font-bold text-body uppercase tracking-wider">Catatan Terisi</p>
                <p class="text-lg font-extrabold text-heading">{{ $hasCatatanCount }} <span class="text-xs font-normal text-body">siswa</span></p>
            </div>
        </div>
        ...
        <div class="p-3.5 bg-white dark:bg-neutral-primary-soft border border-default rounded-base shadow-xs flex items-center gap-3">
            ...
            <div>
                <p class="text-[11px] font-bold text-body uppercase tracking-wider">Belum Ada Catatan</p>
                <p class="text-lg font-extrabold text-heading">{{ $noCatatanCount }} <span class="text-xs font-normal text-body">siswa</span></p>
            </div>
        </div>
    </div>
    ...
    <div class="p-4 rounded-base bg-white dark:bg-neutral-primary-soft border border-default shadow-xs flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div class="flex items-center gap-3 w-full md:w-auto flex-1">
            <div class="relative w-full sm:max-w-md">
                ...
                <input type="text" id="catatan-search" oninput="filterCatatanTable()" class="bg-neutral-secondary-medium border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full ps-9 p-2.5 placeholder:text-body" placeholder="Cari NIS, nama siswa, atau isi catatan..." />
            </div>
        </div>
    </div>
            ...
            <table id="catatan-table" class="w-full text-xs md:text-sm text-left text-body">
                <thead class="text-[11px] md:text-xs font-bold text-heading uppercase bg-neutral-tertiary border-b border-default">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3">Catatan Wali Kelas</th>
                ...
                <tbody class="divide-y divide-default">
                    @forelse($students as $index => $student)
                        @php
                            ...
                            $catatanText = $catatanWaliKelas->catatan ?? '';
                        @endphp
                        <tr class="student-row hover:bg-neutral-secondary-medium/50 transition-colors"
                            data-search="{{ strtolower($student->name . ' ' . $student->nis . ' ' . $student->nisn . ' ' . $catatanText) }}">
                            ...
                            <td class="px-4 py-3 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-1.5">
                                    <button type="button"
                                        onclick="openEditCatatanModal({{ $student->id }}, '{{ addslashes($student->name) }}', '{{ addslashes($catatanText) }}', {{ $catatanWaliKelas->id ?? 'null' }})"
                                        ...
                                        title="Kelola Catatan Wali Kelas">
                                    ...
                                    @if($catatanWaliKelas)
                                        <form action="{{ route('wali-kelas.catatan-wali-kelas.destroy', $catatanWaliKelas->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus catatan wali kelas siswa ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1.5 text-red-600 hover:text-white hover:bg-red-600 border border-red-300 dark:border-red-800 rounded-base transition-colors cursor-pointer" title="Hapus Catatan Wali Kelas">
                                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-8 text-center text-body">
...
<!-- Modal Form Input / Edit Catatan Wali Kelas -->
<div id="catatan-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full flex items-center justify-center bg-black/50 backdrop-blur-xs">
        ...
        <div class="relative bg-white dark:bg-neutral-primary-soft rounded-base shadow-lg border border-default overflow-hidden">
            ...
            <div class="flex items-center justify-between p-4 border-b border-default bg-neutral-tertiary">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-heading">Kelola Catatan Wali Kelas</h3>
                        <p id="modal-student-name" class="text-xs font-semibold text-fg-brand">Nama Siswa</p>
                    </div>
                </div>
                <button type="button" onclick="closeCatatanModal()" class="text-body hover:text-heading rounded-base p-1.5 transition-colors cursor-pointer">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>

            <!-- Modal body -->
            <form id="catatan-form" action="{{ route('wali-kelas.catatan-wali-kelas.store') }}" method="POST" class="p-4 space-y-4 max-h-[75vh] overflow-y-auto">
                @csrf
                <input type="hidden" id="form-student-id" name="student_id" value="">

                <div>
                    <label for="catatan" class="block mb-1 text-xs font-bold text-heading">Catatan Wali Kelas</label>
                    <textarea id="catatan" name="catatan" rows="4" placeholder="Tuliskan catatan perkembangan, motivasi, atau pesan wali kelas untuk siswa..." class="bg-neutral-secondary-medium border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-2.5 transition-colors"></textarea>
                </div>

                <!-- Modal footer -->
                <div class="flex items-center justify-end gap-2 border-t border-default pt-3 mt-4">
                    <button type="button" onclick="closeCatatanModal()" class="px-4 py-2 text-xs font-bold text-body bg-neutral-secondary-medium hover:bg-neutral-tertiary rounded-base border border-default transition-colors cursor-pointer">
                        Batal
                    </button>
...
<script>
    ...
    function filterCatatanTable() {
        const query = document.getElementById('catatan-search').value.toLowerCase().trim();
    ...
    function openEditCatatanModal(studentId, studentName, catatanText, catatanId) {
        document.getElementById('form-student-id').value = studentId;
        document.getElementById('modal-student-name').textContent = studentName;
        document.getElementById('catatan').value = catatanText || '';
