                ...
                <table class="w-full text-sm text-left text-body">
                    <thead class="text-xs font-bold text-heading uppercase bg-neutral-secondary-medium border-b border-default select-none">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3.5">Tanggal Pertemuan</th>
                            <th scope="col" class="px-6 py-3.5">Deskripsi</th>
                            <th scope="col" class="px-6 py-3.5 text-center">Status Terisi</th>
                            <th scope="col" class="px-6 py-3.5 text-center" style="width: 170px;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-default">
                        @forelse($meetings as $meeting)
                            <tr id="meeting-row-{{ $meeting->id }}" data-title="{{ strtolower($meeting->title) }}" data-class="{{ $meeting->class?->name ?? '' }}" data-date="{{ $meeting->meeting_date ? \Carbon\Carbon::parse($meeting->meeting_date)->format('Y-m-d') : '' }}" class="meeting-row hover:bg-neutral-secondary-soft dark:hover:bg-neutral-tertiary transition-colors duration-150 border-b border-default last:border-0 cursor-pointer" onclick="showMeetingAttendances({{ $meeting->id }})">
                                <td class="px-6 py-4 text-center font-semibold text-heading select-none">{{ sprintf('%02d', $loop->iteration) }}</td>
                                ...
                                <td class="px-6 py-4 text-body whitespace-nowrap">{{ $meeting->meeting_date ? \Carbon\Carbon::parse($meeting->meeting_date)->translatedFormat('d F Y') : '-' }}</td>
                                <td class="px-6 py-4 text-body max-w-xs truncate">{{ $meeting->description ?? '-' }}</td>
    ...
    <div id="attendance-fill-section" class="hidden transition-all duration-300">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div class="flex items-center gap-3">
                <button type="button" onclick="backToMeetings()" class="text-body hover:text-brand p-2 rounded-base hover:bg-neutral-secondary-soft border border-default hover:border-brand transition-all duration-200 cursor-pointer flex items-center justify-center shrink-0" title="Kembali ke Daftar Pertemuan">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                    </svg>
                </button>
                <div>
                    <h1 id="attendance-title" class="text-3xl font-extrabold text-heading tracking-tight">Isi Absensi Kelas</h1>
                    <p class="text-body" id="attendance-subtitle">Pertemuan 1 - Kelas VII A</p>
                </div>
            </div>
                ...
                <table class="w-full text-sm text-left text-body">
                    <thead class="text-xs font-bold text-heading uppercase bg-neutral-secondary-medium border-b border-default select-none">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3.5" style="width: 170px;">Status Absensi</th>
                            <th scope="col" class="px-6 py-3.5">Catatan</th>
                            <th scope="col" class="px-6 py-3.5 text-center" style="width: 120px;">Aksi</th>
                        </tr>
                    </thead>
    ...
    function renderAttendances(studentsList) {
        ...
        sortedList.forEach((student, index) => {
            ...
            row.innerHTML = `
                ...
                <td class="px-6 py-4">
                    <select id="status-${student.id}" onchange="markUnsaved(${student.id})" class="bg-neutral-secondary-medium border border-default text-heading text-xs rounded-base focus:ring-brand focus:border-brand block w-28 p-1.5 font-bold">
                ...
                <td class="px-6 py-4">
                    <input type="text" id="note-${student.id}" oninput="markUnsaved(${student.id})" value="${currentNote}" placeholder="Catatan..." class="w-full bg-neutral-secondary-medium border border-default text-heading text-xs rounded-base focus:ring-brand focus:border-brand p-1.5">
                </td>
                <td class="px-6 py-4 text-center">
