...
@section('title', 'Penilaian Akhir Semester')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-body">
                                    ...
                                    <p class="font-medium text-heading">Belum ada data ujian akhir semester.</p>
            ...
            @foreach($exams as $exam)
                <div id="dropdown-{{ $exam->id }}" class="z-50 hidden bg-white divide-y divide-gray-100 rounded-lg shadow-md w-32 dark:bg-neutral-primary-soft dark:divide-neutral-tertiary border border-default text-left">
                    <ul class="py-2 text-sm text-heading" aria-labelledby="dropdownButton-{{ $exam->id }}">
                        ...
                        <li>
                            <form action="{{ route('final-exams.delete', $exam->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus ujian ini? Semua data nilai di ujian ini juga akan terhapus.')" class="block w-full">
                                @csrf
                                @method('DELETE')
    ...
    <div id="score-fill-section" class="hidden transition-all duration-300">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                ...
                <button type="button" onclick="document.getElementById('scan-camera-input').click()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-base text-sm font-bold shadow-md shadow-indigo-600/10 transition-all duration-200 cursor-pointer flex items-center justify-center gap-2">
                ...
                <input type="file" id="scan-camera-input" accept="image/*" capture="environment" class="hidden" onchange="window.scanGrades(this, currentClassStudents)">
            ...
            <form id="exam-form" method="POST" class="p-4 md:p-5 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="modal_title_input" required
                           class="bg-neutral-secondary-medium border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-2.5"
                           placeholder="contoh: PAS Ganjil Matematika">
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="modal_description" rows="3"
                              class="bg-neutral-secondary-medium border border-default text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full p-2.5"
                              placeholder="contoh: Materi Semester 1..."></textarea>
                </div>
...
<script>
    ...
    function prepareAddExam() {
        document.getElementById('modal-title').innerText = 'Tambah Ujian Akhir Semester';
        document.getElementById('exam-form').action = "{{ route('final-exams.store') }}";
    ...
    function prepareEditExam(exam) {
        document.getElementById('modal-title').innerText = 'Ubah Ujian Akhir Semester';
        document.getElementById('exam-form').action = `/final-exams/${exam.id}`;
    ...
    function saveScore(studentId, existingScoreId) {
        ...
        const url = existingScoreId 
            ? `/final-scores/${existingScoreId}`
            : '/final-scores';
            
        const method = existingScoreId ? 'PUT' : 'POST';
        
        const payload = {
            final_exam_id: currentExamId,
    ...
    function saveAllAndFinish(btnElement) {
        ...
        if (unsavedStudents.length === 0) {
            window.location.href = "{{ route('final-exams.index') }}";
            return;
        }
